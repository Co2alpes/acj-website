...
function LandingPage({ onLogin }) {
  ...
  const navBg = isMobileMenuOpen ? 'rgba(255, 255, 255, 0)' : (isScrolled ? 'rgba(255, 255, 255, 1)' : 'rgba(255, 255, 255, 0)');
  const navBorderBottom = (!isMobileMenuOpen && isScrolled) ? '1px solid rgba(200, 200, 200, 0.5)' : '1px solid rgba(255, 255, 255, 0)';
  const navPadding = isScrolled ? '15px 40px' : '30px 50px';
  const contentColor = (isMobileMenuOpen || !isScrolled) ? '#ffffff' : ACJ_COLORS.BLUE;
  const linkColor = (isMobileMenuOpen || !isScrolled) ? '#ffffff' : '#333333';
  const textShadow = (!isMobileMenuOpen && !isScrolled) ? '0 2px 10px rgba(0,0,0,0.5)' : 'none';
  const btnBg = (isMobileMenuOpen || !isScrolled) ? 'rgba(255,255,255,0.2)' : ACJ_COLORS.BLUE;
  const btnText = '#ffffff';
  const btnBorder = (isMobileMenuOpen || !isScrolled) ? '1px solid #ffffff' : `1px solid ${ACJ_COLORS.BLUE}`;
  ...
  const styles = {
    ...
    nav: { 
      ...
      backdropFilter: (isScrolled && !isMobileMenuOpen) ? 'none' : 'blur(0px)'
    },
    logo: { 
    ...
    heroOverlay: { position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, background: 'linear-gradient(to bottom, rgba(0,0,0,0.3) 0%, rgba(13, 71, 161, 0.6) 100%)' },
    ...
    heroBtn: { padding: '18px 45px', backgroundColor: ACJ_COLORS.YELLOW, color: ACJ_COLORS.BLUE, border: 'none', borderRadius: '50px', fontWeight: '800', fontSize: '1rem', cursor: 'pointer', textTransform: 'uppercase', letterSpacing: '1px', boxShadow: '0 10px 30px rgba(255, 193, 7, 0.4)', transition: 'transform 0.2s' },
    ...
    sectionAlt: { padding: '100px 20px', backgroundColor: '#F9FAFB' },
    ...
    sectionSubtitle: { fontSize: '1.1rem', color: '#666', lineHeight: 1.6, textAlign: 'center', maxWidth:'700px', margin:'0 auto 60px auto' },

    servicesGrid: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '30px', maxWidth: '1200px', margin: '0 auto' },
    serviceCard: { padding: '40px 30px', backgroundColor: 'white', borderRadius: '4px', textAlign: 'center', boxShadow: '0 10px 30px rgba(0,0,0,0.05)', borderBottom: `4px solid ${ACJ_COLORS.YELLOW}`, transition: 'transform 0.3s' },
    ...
    footer: { backgroundColor: '#0f151c', color: 'white', padding: '80px 20px 40px 20px' },
    copyright: { textAlign: 'center', marginTop: '60px', paddingTop: '20px', borderTop: '1px solid rgba(255,255,255,0.1)', color: '#555', fontSize: '0.85rem' }
  };
    ...
    <div style={styles.wrapper}>
      ...
      <nav style={styles.nav}>
        ...
        <div className="desktop-menu">
          ...
          <button onClick={onLogin} style={styles.btnLogin}>ESPACE PRO</button>
        </div>
      ...
      <div style={styles.mobileOverlay}>
        ...
        <button onClick={() => {setIsMobileMenuOpen(false); onLogin();}} style={{...styles.btnLogin, backgroundColor:ACJ_COLORS.YELLOW, color:ACJ_COLORS.BLUE, border:'none', marginTop:'20px'}}>ESPACE PRO</button>
      </div>
      ...
      <footer id="contact" style={styles.footer}>
        ...
        <div style={styles.copyright}>
          © {new Date().getFullYear()} ACJ Développement. Tous droits réservés. | <span onClick={onLogin} style={{cursor:'pointer', textDecoration:'underline', color:'white'}}>Connexion Salariés</span>
        </div>
      </footer>
